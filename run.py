...
def strategy(trader: shift.Trader, ticker: str, endtime):
    ...
    buyprice = trader.get_last_price(ticker)
    orderSize = trader.get_portfolio_summary().get_total_bp() // ((buyprice+1) * 100)
    print('buyprice ',buyprice)
    print('ordersize ',int(orderSize))
    

    # strategy parameters
    check_freq = 1
    order_size = orderSize  # NOTE: this is 5 lots which is 500 shares.
    ...
    while (trader.get_last_trade_time() < endtime):
        ...
        for item in trader.get_portfolio_items().values():
            if not item.get_symbol():
                order = shift.Order(shift.Order.Type.LIMIT_BUY, ticker, int(order_size), price=buyprice)
                trader.submit_order(order)
                print('buying')
            elif item.get_symbol():
                order_size = item.get_shares() // 100
                order_price = item.get_price()
                order = shift.Order(shift.Order.Type.MARKET_SELL, ticker, int(order_size), price= order_price)
                trader.submit_order(order)
                print('selling')
...
if __name__ == '__main__':
    with shift.Trader("python_master_test04") as trader:
        trader.connect("initiator.cfg", "j0n5F5j551")
        sleep(1)
